...
async function _sync(_force = false) {
  ...
  try {
    ...
    // ── Remote sync (Beaver-Sync server) ──────────────────────────────────
    // Mirrors the folder sync protocol over HTTP so data is also available
    // on the server for cross-device access. Runs independently of folder sync.
    try {
      const accountStore = useAccountStore();
      if (accountStore.isAuthenticated) {
        // Push: upload files from our own device that haven't been pushed yet
        const ownDeviceId = getSyncDeviceId();
        const ownCursorKey = `yjs-${ownDeviceId}`;
        const ownCursor = cursors[ownCursorKey] || { ts: 0, seq: 0 };
        const flushedFiles = await readSyncDir(commitsDir).catch(() => []);
        const pushBatch = [];
        ...
        for (const upd of remoteUpdates) {
          ...
          applyRemote(payload.noteId, update);
          await appendUpdate(payload.noteId, update, payload.device);

          const cursorKey = `yjs-${payload.device}`;
          const prev = cursors[cursorKey];
          const prevTs = prev?.ts ?? 0;
          const prevSeq = prev?.seq ?? 0;
          const fileTs = payload.ts;
          const fileSeq = parsed?.seq ?? payload.seq ?? 0;
          if (fileTs > prevTs || (fileTs === prevTs && fileSeq > prevSeq)) {
            cursors[cursorKey] = { ts: fileTs, seq: fileSeq };
            remoteCursorsChanged = true;
          }
        }
      ...
      console.warn('[sync] Remote sync failed (non-fatal):', err?.message);
    }
  } catch (err) {
