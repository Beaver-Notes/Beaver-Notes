  ...
  <div class="sm:mb-14 w-full max-w-xl space-y-6">
    ...
    <settings-group :title="translations.settings.behavior || 'Behavior'">
      <settings-row
        ...
        :label="
          translations.settings.openAfterCreation || 'Open note after creation'
        "
        description="Immediately navigate to a note after creating it."
      >
        <ui-switch
          id="general-open-after-creation"
    ...
    <settings-group :title="translations.settings.editor || 'Editor'">
      ...
      <settings-row
        ...
        :label="
          translations.settings.collapsibleHeading || 'Collapsible headings'
        "
        description="Allow headings to be folded so their content is hidden below."
      >
        <ui-switch
          id="general-collapsible-heading"
    ...
    <settings-group v-if="isIOSRuntime" title="Spotlight">
      ...
      >
        <ui-switch
          ...
          v-model="spotlightEnabled"
          @change="toggleSpotlight"
        />
      </settings-row>
    </settings-group>
    ...
    <settings-group :title="translations.settings.security || 'Security'">
      <div class="px-4 py-3.5 space-y-3">
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1 min-w-0">
            <span
              ...
              >{{ translations.settings.encryption || 'Encryption' }}</span
            >
            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
              {{
                translations.settings.encryptionDesc ||
                'All notes and assets are encrypted at rest. Encryption is always on.'
              }}
            </p>
          </div>
          <span
        ...
        <transition name="setting-fade">
          <div v-if="encryptionBusy" class="rounded-xl dark:bg-primary/10 p-2">
            <p class="text-xs text-primary">
              {{ encryptionProgressLabel }}:
              {{ encryptionProgress.processed }} /
              {{ encryptionProgress.total }} ({{ encryptionProgressPercent }}%)
            </p>
            ...
            <div class="mt-2 h-1.5 rounded bg-primary/70 dark:bg-primary/20">
              <div
                ...
                :style="{
                  transform: `scaleX(${encryptionProgressPercent / 100})`,
                }"
              />
            </div>
          </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <details>
            <summary
              class="text-sm cursor-pointer text-neutral-500 dark:text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-300"
            >
    ...
    >
      ...
      >
        ...
        <p
          class="mt-0.5 text-xs leading-relaxed text-neutral-500 dark:text-neutral-400"
        >
          Sync through Beaver Sync (instead of a folder).
        </p>
      ...
      >
        <div class="min-w-0">
          <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">
            {{ trExport.export || 'Export' }}
          </p>
          <p class="text-xs text-neutral-500 dark:text-neutral-400">
            Back up or convert all notes.
          </p>
        </div>
        <ui-button @click="openExportModal()">{{
          trExport.export || 'Export'
        }}</ui-button>
      </div>
      <div
      ...
      >
        <div class="min-w-0">
          <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">
            {{ trExport.import || 'Import' }}
          </p>
          <p class="text-xs text-neutral-500 dark:text-neutral-400">
            Restore a backup or bring notes from other apps.
          </p>
        </div>
        <ui-button @click="openImportModal('beaverBackup')">{{
          trExport.import || 'Import'
        }}</ui-button>
      </div>
    </settings-group>
    ...
    <ui-modal
      ...
      :title="trExport.export || 'Export'"
      icon="riFileDownloadLine"
    >
      <p class="mb-4 text-neutral-600 leading-relaxed dark:text-neutral-200">
        {{
          trExport.exportDescriptionLong ||
          'Back up everything as a Beaver Notes archive, or convert all notes to Markdown or HTML.'
        }}
      </p>
      <div class="space-y-1">
        <div v-for="option in exportOptions" :key="option.key">
          ...
          >
            ...
            <p
              v-if="option.key === 'backup' && backupExportState.running"
              class="text-xs text-neutral-500 dark:text-neutral-400"
            >
              {{ trExport.creatingBackup || 'Creating backup…' }}
            </p>
            <div
              v-else-if="
                option.key !== 'backup' && bulkExportState(option.key).running
              "
      ...
      <template #actions>
        <ui-button
          ...
          >{{ translations.dialog?.cancel || 'Cancel' }}</ui-button
        >
        <ui-button
          class="w-full"
          variant="primary"
          ...
          >{{ activeExportOption?.buttonLabel }}</ui-button
        >
      </template>
    </ui-modal>
    ...
    <ui-modal
      ...
      content-class="max-w-md"
      :title="trExport.import || 'Import'"
      icon="riFileUploadLine"
    >
      <p class="mb-4 text-neutral-600 leading-relaxed dark:text-neutral-200">
        {{
          trExport.importDescriptionShort ||
      ...
      <div class="max-h-[24rem] space-y-1 overflow-y-auto">
        <template v-for="group in importSourceGroups" :key="group.label">
          <p
            class="px-3 pb-1 pt-3 text-xs font-semibold font-bold tracking-wider text-neutral-400 dark:text-neutral-500"
          >
            {{ group.label }}
          </p>
            ...
            >
              ...
              <div v-if="source.key === 'evernote'" class="space-y-1">
                <label
                  ...
                  >{{ trExport.notebookName || 'Notebook name' }}</label
                >
                <ui-input
                  v-model="importState.evernote.notebookName"
                  :placeholder="
                    trExport.notebookNamePlaceholder ||
      ...
      <template #actions>
        ...
        <ui-button
          ...
          >{{ activeImportSource?.buttonLabel }}</ui-button
        >
      </template>
    </ui-modal>
    ...
    <settings-group v-if="isDebugMode" title="Debug" danger>
      <div class="px-4 py-3.5 flex flex-col gap-3">
        <div class="space-y-0.5">
          ...
          >
            Debug-only reset that wipes local state and relaunches into a fresh
            install.
          </p>
        </div>
        <ui-button class="w-full" variant="danger" @click="nukeAppDebugOnly"
          >Nuke app and restart</ui-button
        >
      </div>
    </settings-group>
  </div>
...
export default {
  ...
  setup() {
    ...
    const exportOptions = [
      {
        ...
        icon: 'riArchiveLine',
        description:
          'Full backup as a dated archive folder. Use to move devices or keep backups.',
      ...
      },
      {
        ...
        title: 'Markdown',
        icon: 'riMarkdownLine',
        description: 'Export all notes as .md files with YAML frontmatter.',
      ...
      },
      {
        ...
        icon: 'riGlobalLine',
        description:
          'Export all notes as .html files preserving folder structure.',
    ...
    const activeExportOption = computed(
      () =>
        exportOptions.find((o) => o.key === selectedExportKey.value) || null,
    );
    const activeImportSource = computed(
      () =>
        importSources.value.find((s) => s.key === selectedImportSource.value) ||
        null,
    );
    ...
    function openExportModal(key = selectedExportKey.value) {
      if (exportOptions.some((o) => o.key === key))
    ...
    const encryptionProgressLabel = computed(() => {
      switch (encryptionProgress.value.phase) {
        case 'decrypt':
          return (
            translations.value.settings?.decryptingExistingNotes ||
            'Decrypting existing notes'
          );
        case 'encrypt':
          return (
            translations.value.settings?.encryptingNotes || 'Encrypting notes'
          );
        case 'assets-encrypt':
          return (
            translations.value.settings?.encryptingAssets || 'Encrypting assets'
          );
        default:
          return (
            translations.value.settings?.processingNotes || 'Processing notes'
          );
      }
    });
