...
fn parse_evernote_resources(note_block: &str) -> Result<Vec<ImportResourcePayload>, String> {
  extract_tag_blocks(note_block, "resource")
    ...
    .map(|resource_block| {
      ...
      let mime = decode_xml_entities(&extract_tag_value(&resource_block, "mime").unwrap_or_default());
...
fn parse_evernote_note(
  ...
  let created_at = parse_enex_timestamp(
    &extract_tag_value(note_block, "created").unwrap_or_default(),
  );
  let updated_at = parse_enex_timestamp(
    &extract_tag_value(note_block, "updated").unwrap_or_default(),
  );
  ...
  let content = replace_en_media(
    replace_en_todos(replace_en_note_tags(strip_processing_instructions(raw_content))),
    &resources,
  );
...
#[tauri::command]
pub(crate) fn migration_run(app: AppHandle) -> Result<LegacyMigrationResult, String> {
  run_legacy_store_data_migration(&app)
}
        ...
        let _ = app_handle.emit_to(
          ...
          ImportCompletePayload {
            ...
            errors: vec![ImportErrorPayload {
              ...
              reason: String::from_utf8_lossy(&command_output.stderr).trim().to_string(),
            }],
          },
        );
        return;
