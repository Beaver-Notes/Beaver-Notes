...
pub(crate) fn ensure_store(
    app: &AppHandle,
    path: &str,
) -> Result<Arc<tauri_plugin_store::Store<Wry>>, String> {
...
pub(crate) fn get_settings_value(app: &AppHandle, state: &AppState, key: &str) -> Option<Value> {
    if let Ok(pool) = settings_pool(app, state) {
        if let Ok(Some(raw)) = crate::db::db_get(pool, key) {
            return serde_json::from_str(&raw).ok();
        }
    }
    ensure_store(app, SETTINGS_STORE)
        .ok()
