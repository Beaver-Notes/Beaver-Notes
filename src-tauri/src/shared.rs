...
pub(crate) fn get_settings_value(app: &AppHandle, key: &str) -> Option<Value> {
  ensure_store(app, SETTINGS_STORE).ok().and_then(|store| store.get(key))
}

pub(crate) fn get_data_dir(app: &AppHandle) -> Result<PathBuf, String> {
  if let Some(Value::String(value)) = get_settings_value(app, "dataDir") {
...
pub(crate) fn path_for_name(app: &AppHandle, name: &str) -> Result<PathBuf, String> {
  match name {
    ...
    "desktop" => app.path().desktop_dir().map_err(to_error),
...
pub(crate) fn resolve_asset_path_from_protocol_url(
  ...
  let root_name = match scheme {
    ...
    _ => return Err(format!("Unsupported asset scheme: {scheme}")),
  };
  let base = get_data_dir(app)?.join(root_name);
...
fn stronghold_snapshot_path(app: &AppHandle) -> Result<PathBuf, String> {
  Ok(get_data_dir(app)?.join(STRONGHOLD_SNAPSHOT_FILE))
}
...
pub(crate) fn app_crypto_salt_hex(app: &AppHandle) -> Result<String, String> {
  let salt_path = get_data_dir(app)?.join("app-crypto").join("salt");
  fs::read_to_string(salt_path).map_err(to_error)
}

pub(crate) fn current_asset_passphrase(app: &AppHandle, state: &AppState) -> Result<String, String> {
  ...
  let passphrase = keyring_entry(APP_PASSPHRASE_ACCOUNT)?
    .get_password()
    .map_err(to_error)?;
...
pub(crate) fn sync_roots_from_settings(app: &AppHandle, state: &AppState) {
  for key in ["syncPath", "defaultPath", "default-path", "dataDir"] {
    if let Some(Value::String(value)) = get_settings_value(app, key) {
      if !value.trim().is_empty() {
...
pub(crate) fn protocol_response(status: StatusCode, path: &Path, bytes: Vec<u8>) -> Response<Vec<u8>> {
  Response::builder()
    .status(status)
